...
#   css/*
#
# @file       update_course_with_mod_temp.sh
# @author     upsetrobot
...
echo 'This script will update all modules in the course with the files from '
...
cp -r .css $COURSE_DIR
cp -r .css $COURSE_DIR/supplemental
...
do 
    ...
    then 
        ...
        sed -i "s/<Topic>/$(basename $(dirname $i) | tr '_' ' ' | sed 's/ [a-z]/\U&/g')/" $i/lab_verification_test.py
    fi
done
